...
var funUpdatePassword = async function (userID, userOldPwd, userNewPwd) {

    var ldapUser = ldapReferences[5] + '=' + userID + ',' + ldapReferences[2];
    var ldapPassword = userOldPwd;
    var result;
    ...
    try {
        ...
        */
        ...
        var ldapClient = objLDAP.createClient({
            url: ldapReferences[0]
        });

        ldapClient.bind(ldapUser, ldapPassword, function (err, result) {
            if (err) {
                console.error('aqui error: ' + err);
                return JSON.parse(objStdMessages.stdMessages[1].replace('__RESULT__', '"Please check back-end logs: ' + err.message + '"'));
            } else {
                console.log('estou logado com ' + ldapUser);
                ldapClient.modify(ldapUser, [
                    new objLDAP.Change({
                        operation: 'delete',
                        modification: {
                            userPassword: (userOldPwd)
                        }
                    }),
                    new objLDAP.Change({
                        operation: 'add',
                    ...
                    if (err) {
                        ...
                        //return JSON.parse(objStdMessages.stdMessages[0].replace('__RESULT__', '"Password changed!"'));
                        ldapClient.unbind(function(){
                            return JSON.parse(objStdMessages.stdMessages[1].replace('__RESULT__', '"Please check back-end logs: ' + 'teste' + '"'));
                        });
                    }
                });
            }
...
module.exports = {
    ...
    doChangePassword: funUpdatePassword
}